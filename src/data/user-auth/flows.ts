...
const MASTER_FLOW = `flowchart TD
    ...
    REQ["POST /api/v1/auth/phone/request-otp\\n{ countryCode, phoneNumber }"] --> REQ_OK{"200 OK?"}
    REQ_OK -->|"No - bad phone"| REQ400["400 Bad Request\\npattern ^[0-9]{9,10}$ violated"]
    REQ_OK -->|"Yes"| REQ_SUCC["{ token, refNo, expiredAt }\\nOTP session created"]
    REQ_SUCC --> VER["POST /api/v1/auth/phone/verify-otp\\n{ providerType, token, pin: '123456' }"]
    VER --> VER_OK{"200 OK?"}
    VER_OK -->|"No - wrong pin"| VER401["401 Unauthorized\\npin not bypass and not real OTP"]
...
const FLOW_REGISTER = `flowchart TD
    S1(["New phone\\ntimestampPhone()"]) --> S2["POST /api/v1/auth/phone/request-otp\\n{ countryCode, phoneNumber }"]
    S2 --> S3{"200 OK?"}
    S3 -->|"UA-REG-TC-004~008"| S4["400 Bad Request\\nphone pattern violated"]
    S3 -->|"UA-REG-TC-001~003"| S5["{ token, refNo, expiredAt }\\nOTP session created"]
    S5 --> S6["POST /api/v1/auth/phone/verify-otp\\n{ providerType, token, pin: '123456' }"]
    S6 --> S7{"200 OK?"}
...
const FLOW_LOGIN = `flowchart TD
    S1(["Registered phone\\n+ bypass pin=123456"]) --> S2["POST /api/v1/auth/phone/verify-otp\\n{ providerType, token, pin }"]
    S2 --> S3{"200 OK?"}
    S3 -->|"UA-LOGIN-TC-001"| S4["{ accessToken, refreshToken\\nisRegistered }"]
    S4 --> S5["GET /user/profile\\nUA-LOGIN-TC-002"]
    S4 --> S6["POST /api/v1/auth/refresh\\nUA-LOGIN-TC-003"]
    S5 --> S7(["200 OK - profile data"])
    S6 --> S8(["200 OK - new accessToken"])
    S3 -->|"UA-LOGIN-TC-004"| S9["401 - wrong pin"]
    S3 -->|"UA-LOGIN-TC-005~007"| S10["400 - pin pattern violation"]
    S3 -->|"UA-LOGIN-TC-008"| S11["Behavior under review\\nunregistered phone path"]
    S3 -->|"UA-LOGIN-TC-009~014"| S12["400 - request/body schema error"]
...
const FLOW_REFRESH = `flowchart TD
    S1(["refreshToken obtained\\nfrom verify-otp"]) --> S2["POST /api/v1/auth/refresh\\n{ refreshToken }"]
    S2 --> S3{"200 OK?"}
    S3 -->|"UA-LOGIN-TC-003"| S4(["{ accessToken, refreshToken, isRegistered }\\ntoken rotated"])
...
export const MASTER_FLOW_SECTION: FlowSectionDef = {
  ...
  subtitle: 'request-otp → verify-otp → profile / refresh',
  chart: MASTER_FLOW,
};
...
export const FLOW_SECTIONS: FlowSectionDef[] = [
  {
    ...
    states: [
      ...
      ['S5',  '200 OK — { token, refNo, expiredAt }'],
      ['S6',  'POST verify-otp sent (bypass pin=123456)'],
    ...
    transitions: [
      ...
      ['T3', 'bypass pin=123456 after request-otp → verify-otp 200'],
  ...
  },
  {
    ...
    chart: FLOW_LOGIN,
    states: [
      ['S1',  'Registered phone + bypass pin'],
      ['S2',  'POST verify-otp sent'],
      ['S3',  'verify-otp response check'],
      ['S4',  '200 OK — { accessToken, refreshToken, isRegistered }'],
      ['S5',  'GET profile with accessToken'],
      ['S6',  'POST refresh with refreshToken'],
      ['S7',  '200 OK — profile data'],
      ['S8',  '200 OK — new accessToken'],
      ['S9',  '401 — wrong pin (non-bypass)'],
      ['S10', '400 — pin pattern violation (length/format)'],
      ['S11', 'Behavior under review — unregistered phone path'],
      ['S12', '400 — request/body schema error'],
    ],
    transitions: [
      ['T1', 'bypass pin=123456 on registered phone → 200 token contract'],
      ['T2', 'accessToken valid → GET profile 200'],
      ['T3', 'refreshToken valid → POST refresh 200'],
      ['T4', 'wrong pin (pin=000000) → 401'],
      ['T5', 'pin length outside 6 digits → 400'],
      ['T6', 'non-numeric pin → 400'],
      ['T7', 'unregistered phone path requires clarification'],
      ['T8', 'missing providerType / token / pin / empty body → 400'],
      ['T9', 'request-otp setup missing countryCode / phoneNumber → 400'],
    ],
  },
  {
    ...
    subtitle: 'UA-LOGIN-TC-003 · POST /api/v1/auth/refresh',
    chart: FLOW_REFRESH,
    states: [
      ['S1', 'refreshToken obtained from verify-otp response'],
      ['S2', 'POST refresh sent'],
      ['S3', 'Response check'],
      ['S4', '200 OK — { accessToken, refreshToken, isRegistered } — token rotated'],
