...
const SYSTEM_PROMPT = `You are GPT, a QA test design assistant embedded in
...
Rules: under 180 words. Never include @GPT in your response.
If request is vague, use [QUESTION].`;

function runGpt(userMessage: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code: number | null) => {
      if (code !== 0) reject(new Error(err.slice(0, 300)));
...
export async function triggerGptResponse(
  storyId: string,
  tcId: string,
  ...
  const text = await runGpt(userMessage);
  if (!text) return;

  await addComment(storyId, tcId, 'GPT', text);
  emitRefresh(storyId);
}
